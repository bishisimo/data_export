...
class OracleJob:
    ...
    def export(self,cursor,table_name,table_dict,target_time,deadline_time,mode='daily'):
        ...
        try:
            ...
            df.to_parquet(f"{data_dir}{file_name}.snappy.parquet", index=False)
    ...
    def export_daily(self):
        ...
        for table_name in self.config['tables']:
            ...
            #     deadline_str = deadline_time.format('YYYY-MM-DD HH:mm:ss')
            # try:
            #     sql_context = f"select * from {table_name} where {table_dict['field']}>=TO_DATE('{target_str}','{oracle_format}') and {table_dict['field']}<TO_DATE('{deadline_str}','{oracle_format}')"
            #     if 'where' in table_dict and table_dict['where'] != '':
            #         sql_context += f'and {table_dict["where"]}'
            #     cursor.execute(sql_context)
            #     rows = cursor.fetchall()
            ...
            #     if not os.path.exists(data_dir):
            #         os.makedirs(data_dir)
            #     df.to_parquet(f"{data_dir}{target_time.format('YYYY-MM-DD')}.snappy.parquet", index=False)
            #     if table_name not in self.record:
            #         self.record[table_name] = {}
            #     self.record[table_name]['daily_time'] = target_str
            #     with open('./record.yaml', 'w') as f:
            #         yaml.safe_dump(self.record, f)
            #     logger.info(
            #         f'table={table_name},len={len(df)},target_datetime={target_str}')
        ...
        for table_name in self.config['tables']:
            ...
            try:
                ...
                while True:
                    ...
                    # if not os.path.exists(data_dir):
                    #     os.makedirs(data_dir)
                    # df.to_parquet(f"{data_dir}{target_time.format('YYYY-MM')}.snappy.parquet", index=False)
                    # if table_name not in self.record:
                    #     self.record[table_name] = {}
                    # self.record[table_name]['history_time'] = target_str
                    # with open('./record.yaml', 'w') as f:
                    #     yaml.safe_dump(self.record, f)
                    # logger.info(
                    #     f'table={table_name},len={len(df)},target_datetime={target_str}')
    ...
    def history_export_daily(self):
        cursor = self.get_cursor()
        now=arrow.now()
        for table_name in self.config['tables']:
            target_time = now.floor('day')
            try:
                table_dict = self.config['tables'][table_name]
                if table_name not in self.ancient:
                    self.ancient[table_name] = self._get_ancient_time(table_name, table_dict['field'])
                while True:
                    ...
                    if ('hd_time' not in self.record[table_name] or self.record[table_name]['hd_time']=='')and target_time<=arrow.get(self.record[table_name]['hd_time'],tzinfo='local') or target_time <= now.floor('month'):
                        break
